...
# Helper function to calculate the percentile value
percentile <- function(pct, items) {
  return(items[min(length(items), floor(length(items) * pct / 100))])
}

# Helper function to create histograms
hist_plot <- function(data, title, range) {
  ggplot(data.frame(x = data), aes(x = x)) + 
    geom_histogram(binwidth = diff(range) / 30, fill = "blue", color = "black") + 
    xlim(range) + 
    ggtitle(title) + 
    xlab("Wealth") + 
    ylab("Frequency") +
...
show <- function(population, k = 40, percentiles = c(1, 10, 50, 90, 99), ...) {
  ...
  for (res in results) {
    if (res$t %% (k * N / 10) == 0) {
      pct_values <- sapply(percentiles, function(pct) percentile(pct, res$pop))
...
Disclosure: A lot of this assignment was created by at least consulting ChatGPT